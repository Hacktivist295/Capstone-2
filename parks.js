...
// junk
const parkDetailsElement = document.getElementById('park-details');
...
function generateLocationOptions() {
    ...
    allOption.value = 'all';
    allOption.textContent = 'All Parks';
...
function generateParkTypeOptions() {
...
function displayParkDetails(park) {
    parkDetailsElement.innerHTML = '';
    if (park) {
        const details = document.createElement('div');
        details.innerHTML = `
          <h3>${park.LocationName}</h3>
          <p><strong>Address:</strong> ${park.Address}, ${park.City}, ${park.State}</p>
        ...
        `;
        parkDetailsElement.appendChild(details);
    }
}
...
function displayAllParks() {
    ...
    parks.forEach(park => {
        ...
        parkDiv.innerHTML = `<h3>${park.LocationName}</h3>`;
        parkDetailsElement.appendChild(parkDiv);
    });
}
...
locationSelect.addEventListener('change', () => {
    ...
    if (selectedLocation === 'all') {
        ...
        const filteredParks = parks.filter(park => park.State === selectedLocation);
        const selectedPark = filteredParks.length > 0 ? filteredParks[0] : null;
        displayParkDetails(selectedPark);
    }
});

// Event listener for park type selection
typeSelect.addEventListener('change', () => {
    const selectedType = typeSelect.value;

    const filteredParks = parks.filter(park => park.LocationID === selectedType);
    const selectedPark = filteredParks.length > 0 ? filteredParks[0] : null;
    displayParkDetails(selectedPark);
});

// Generate location and park type options on page load
generateLocationOptions();
generateParkTypeOptions();